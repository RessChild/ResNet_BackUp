...
# 이 블록층을 사용하므로, 인자를 좀 수정 @@
def _building_block_v2(inputs, filters, training, projection_shortcut, strides, activation_type,
                       data_format, set_idx, val_idx):
  ...
  if activation_type:
      ...
      inputs = tf.keras.activations.elu(inputs, alpha = 1.0)
...
def block_layer(inputs, filters, bottleneck, block_fn, blocks, strides, activation_type,
                training, name, data_format, set_idx):
  ...
  inputs = block_fn(inputs, filters, training, projection_shortcut, strides,activation_type,
                    data_format, set_idx, 0)

  for val_idx in range(1, blocks):
    inputs = block_fn(inputs, filters, training, None, 1, activation_type, data_format, set_idx, val_idx)
...
class Model(object):
  ...
  def __init__(self, resnet_size, bottleneck, num_classes, num_filters,
               ...
               block_sizes, block_strides, activation_type,
  ...
  def __call__(self, inputs, training):
    ...
    with self._model_variable_scope():
      ...
      # 이부분도, 상위에서 호출하는 방식아라서 block_layer에 i 인자를 추가로 넘겨줌 @@
      for i, num_blocks in enumerate(self.block_sizes):
        num_filters = self.num_filters * (2**i)
        inputs = block_layer(
            inputs=inputs, filters=num_filters, bottleneck=self.bottleneck,
            block_fn=self.block_fn, blocks=num_blocks,
            strides=self.block_strides[i], activation_type = self.activation_type[i],
            training=training,
            # 활성화 타입 @@
            name='block_layer{}'.format(i + 1), data_format=self.data_format, set_idx=i)
